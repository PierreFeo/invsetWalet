...
//    //TODO i stopped here
    ...
    @Transaction
    @Query("UPDATE accounts_table SET total=(SELECT SUM(` amount_transaction`) FROM currency_transactions_table WHERE account_id=:id), result=(SELECT SUM(resultTransaction)FROM currency_transactions_table WHERE account_id=:id) WHERE id in(:id)")
